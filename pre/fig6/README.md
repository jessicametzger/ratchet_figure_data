...
Then, the data is parsed and averaged using the script ``data/avg_data.py``. The averaged current for ``N=X`` is stored in the file ``data/NX/J_avg``. The averaged displacement vs. time is stored in ``data/NX/amp-Y-avg_disp`` for interaction strength ``Y``. The averaged density is stored in ``data/NX/amp-Y-avg_profile_rho`` for interaction strength ``Y``. The averaged magnetization profile is stored in ``data/NX/amp-Y-avg_profile_m`` for interaction strength ``Y``. The averaged density is stored in ``data/NX/amp-Y-avg_profile_rho`` for interaction strength ``Y``. A copy of the parsed and averaged data is included here for each $N$.
