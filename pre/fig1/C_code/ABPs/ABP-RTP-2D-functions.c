...
void Initialize_Parameters(int argc, char* argv[], param* Param, state* State, data* Data, particle** Particles
  ...
  */
  ...
  Data[0].Jprof            = (double***) malloc(Param[0].Nbinx*sizeof(double**));
  for (i=0;i<Param[0].Nbinx;i++){
    Data[0].Jprof[i]       = (double**) calloc(Param[0].Nbiny,sizeof(double*));
    for (j=0;j<Param[0].Nbiny;j++){
      Data[0].Jprof[i][j]  = (double*) calloc(5, sizeof(double));
      Data[0].Jprof[i][j][0] = 0.0;  // fx*dt due to interactions
      Data[0].Jprof[i][j][1] = 0.0;  // fy*dt due to interactions
      Data[0].Jprof[i][j][2] = 0.0;  // dx
      Data[0].Jprof[i][j][3] = 0.0;  // dy
    }
  }
...
#endif
    Data[0].Jprof[j][k][2] += Particles[i].dx;
    Data[0].Jprof[j][k][3] += Particles[i].dy;
  }
}

void Store_J(param Param,state State, data* Data){
  int n,j,k; // profile index
