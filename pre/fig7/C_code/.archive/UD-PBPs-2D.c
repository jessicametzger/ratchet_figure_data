...
#ifndef NI 
  FILE* output_sigmaIKprof;   // file where profile of irving-kirkwood stress tensor is stored
#endif
  FILE* output_sigmaprof;     // file where profile of stress tensor is stored
} param;
...
typedef struct particle
{
  ...
  double px;
  double py;    // momentum of particles
  double dx;    // x increment
  double dy;    // y increment
  double dpx;   // px increment
  double dpy;   // py increment
...
typedef struct data{
  ...
  double*** sigmaIKprof;           // sigmaIKprof[i][j][k] = irving-kirkwood stress component k at xi,xj
  double*** sigmaprof;             // sigmaprof[i][j][k] = stress component k at xi,xj
} data;

#ifndef NI 
#include "PFs.c"
#endif
#include "./UD-PBPs-2D-functions.c" // our methods
