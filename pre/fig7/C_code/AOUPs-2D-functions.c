...
void Initialize_parameters (int argc, char *argv[], param* Param, state* State, data* Data, particle** Particles
  ...
  for (i=0; i<Param[0].NxBox; i++){
    ...
    for (j=0; j<Param[0].NyBox; j++){
      Data[0].sigmaIKprof[i][j] = calloc(3, sizeof(double));
  ...
  Data[0].sigmaAprof = calloc(Param[0].Nbinx, sizeof(double**));
  for (i=0; i<Param[0].Nbinx; i++){
    Data[0].sigmaAprof[i] = calloc(Param[0].Nbiny, sizeof(double*));
    for (j=0; j<Param[0].Nbiny; j++){
      Data[0].sigmaAprof[i][j] = calloc(8, sizeof(double));
      Data[0].sigmaAprof[i][j][0] = 0.0; // rho*D/tau
      Data[0].sigmaAprof[i][j][1] = 0.0; // vx*vx
      Data[0].sigmaAprof[i][j][2] = 0.0; // vx*vy
      Data[0].sigmaAprof[i][j][3] = 0.0; // vy*vy
      Data[0].sigmaAprof[i][j][4] = 0.0; // vx*Fx
      Data[0].sigmaAprof[i][j][5] = 0.0; // vx*Fy
      Data[0].sigmaAprof[i][j][6] = 0.0; // vy*Fx
      Data[0].sigmaAprof[i][j][7]= 0.0; // vy*Fy
    }
  }
...
void Update_SigmaA(param Param, particle* Particles, data* Data){
  ...
  for (i=0; i<Param.N; i++){
    j = (int) (floor(Particles[i].x/Param.binwidthx)+EPS);
    k = (int) (floor(Particles[i].y/Param.binwidthy)+EPS);
    Data[0].sigmaAprof[j][k][0] += Particles[i].D/Param.tau;
    Data[0].sigmaAprof[j][k][1] += Particles[i].vx*Particles[i].vx;
    Data[0].sigmaAprof[j][k][2] += Particles[i].vx*Particles[i].vy;
    Data[0].sigmaAprof[j][k][3] += Particles[i].vy*Particles[i].vy;
#ifndef NI
    Data[0].sigmaAprof[j][k][4] += Particles[i].vx*Particles[i].ifx;
    Data[0].sigmaAprof[j][k][5] += Particles[i].vx*Particles[i].ify;
    Data[0].sigmaAprof[j][k][6] += Particles[i].vy*Particles[i].ifx;
    Data[0].sigmaAprof[j][k][7] += Particles[i].vy*Particles[i].ify;
#endif
  }
}

void Store_SigmaA(param Param,particle* Particles, state State, data* Data) {
  int n,j,k; // profile index
...
void Store_Data(param Param, particle* Particles, state* State, data* Data){
  ...
  if (State[0]._time > State[0].NextStoreProf-EPS){
    ...
    Store_SigmaA(Param, Particles, State[0], Data);
#ifndef NI 
