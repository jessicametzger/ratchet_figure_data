...
typedef struct param {
  ...
  double dt_tau;              // dt/tau
  double sqrt2dt_tau;

  fparam_cub DParam;          // parameters of temperature landscape (included in inhom_fluct.c)
  char* Ds;
  char* Dxs;
  ...
  FILE* output_sigmaAprof;   // file where profile of stress tensor is stored
...
typedef struct particle
{
  ...
  double vx;
  double vy;    // momentum of particles
  double dx;    // x increment
  double dy;    // y increment
  double dvx;   // px increment
  double dvy;   // py increment
...
typedef struct data{
  ...
  double*** sigmaAprof;            // sigmaAprof[i][j][k] = stress component k at xi,xj
...
#include "util/PFs.c"
#endif
#include "./AOUPs-2D-functions.c" // our methods
