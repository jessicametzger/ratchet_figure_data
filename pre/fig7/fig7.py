...
sigA_avg = np.genfromtxt(os.path.join(folders[0],'sigA_avg'),delimiter='\t')
...
sigA_avg = sum([sigA_avg[i::binw,:] for i in range(binw)])/binw
...
ax1.plot(xs_data+dx/2.0, (1/fofx)/np.trapezoid(1/fofx,xs_data), color='tan',lw=1.,label=r'$\propto 1/D(x)$')
...
ax3.plot(xs_sig+dx_sig/2.0, sigIK_avg[:,0]+sigA_avg[:,1]+sigA_avg[:,4],lw=2., color=c_stot,
...
ax3.plot([-1,-1],[0,0],color='white',alpha=0,label=' ')
ax3.plot(xs_sig+dx_sig/2.0, sigA_avg[:,1], color=c_sA1, lw=2., label=r'$\langle (v^x)^2\rangle$')
ax3.plot(xs_sig+dx_sig/2.0, sigA_avg[:,4], color=c_sA2, lw=2., label=r'$\langle F^x v^x\rangle$')
ax3.plot([-1,p.Lx+1],[0,0],lw=0.3,color='black')

# plot total xx and yy stresses
ax4.plot(xs_sig+dx_sig/2.0, sigIK_avg[:,0]+sigA_avg[:,1]+sigA_avg[:,4],lw=2., color=c_stot,
         label=r'$-\sigma_{\rm tot}^{xx}$')
ax4.plot(xs_sig+dx_sig/2.0, sigIK_avg[:,2]+sigA_avg[:,3]+sigA_avg[:,7],lw=2., color=c_stoty, 
...
ax2.tick_params(axis='y',which='both',pad=-18)
...
ax3.set_ylim(ymax=0.31,ymin=-.16)
ax4.set_ylim(ymax=0.16,ymin=0.075)
...
# axis labels
ax0.set_ylabel(r'$D(x)$',rotation='horizontal')
