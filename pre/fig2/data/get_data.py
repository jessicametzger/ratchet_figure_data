...
for f in folders:
    ...
    if 'nonint' not in f:
        ...
        # thermal stress
        sigms_ = [np.genfromtxt(x+'-sigmaprof',delimiter='\t')[:,1]/p.NstepProf/p.N/dx for x in seeds]
        sigms_ = [np.array([x[p.Nbinx*i:p.Nbinx*(i+1)] for i in range(3)]) for x in sigms_]
        sigm = np.transpose(sum(sigms_) / len(seeds))
        ...
        # save stress data
        np.savetxt(os.path.join(f,'sigm_avg'),sigm,delimiter='\t',fmt='%.4e')
