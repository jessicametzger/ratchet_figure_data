...
void Initialize_parameters (int argc, char *argv[], param* Param, state* State, data* Data, particle** Particles
  ...
  strcat (command_base, "gamma ");                   argctarget ++;
  strcat(command_base, "Nstep_T ");                  argctarget ++;
  strcat(command_base, "Txs ");                      argctarget ++;
  ...
  sprintf(name,"%s-sigmaprof",argv[i]);
  Param[0].output_sigmaprof=fopen(name,"w");
  ...
  Param[0].gamma              = strtod (argv[i], NULL)             ; i++ ;
  ...
  Param[0].gamma_dt        = Param[0].gamma*Param[0].dt;
  ...
  Param[0].sqrt2gammadt         = pow(2*Param[0].gamma*Param[0].dt, 0.5);
  Param[0].TParam.cs            = (double*) calloc(Param[0].TParam.Nstep, sizeof(double));
  Param[0].TParam.ds            = (double*) calloc(Param[0].TParam.Nstep, sizeof(double));
  for (i=0; i<Param[0].TParam.Nstep-1; i++){
    tmp                         = Param[0].TParam.xs[i+1] - Param[0].TParam.xs[i];
    Param[0].TParam.cs[i]       = -3 * (Param[0].TParam.fs[i] - Param[0].TParam.fs[i+1]) / pow(tmp,2);
    Param[0].TParam.ds[i]       = 2 * (Param[0].TParam.fs[i] - Param[0].TParam.fs[i+1]) / pow(tmp,3);
  }
  tmp                           = Param[0].TParam.xs[0] + Param[0].Lx - Param[0].TParam.xs[i];
  Param[0].TParam.ds[i]         = 2 * (Param[0].TParam.fs[i] - Param[0].TParam.fs[0]) / pow(tmp,3);
  Param[0].TParam.cs[i]         = -3 * (Param[0].TParam.fs[i] - Param[0].TParam.fs[0]) / pow(tmp,2);
  ...
  for (i=0; i<Param[0].NxBox; i++){
    ...
    for (j=0; j<Param[0].NyBox; j++){
      Data[0].sigmaIKprof[i][j] = calloc(7, sizeof(double));
  ...
  Data[0].sigmaprof = calloc(Param[0].Nbinx, sizeof(double**));
  for (i=0; i<Param[0].Nbinx; i++){
    Data[0].sigmaprof[i] = calloc(Param[0].Nbiny, sizeof(double*));
    for (j=0; j<Param[0].Nbiny; j++){
      Data[0].sigmaprof[i][j] = calloc(7, sizeof(double));
      Data[0].sigmaprof[i][j][0] = 0.0; // px*px
      Data[0].sigmaprof[i][j][1] = 0.0; // px*py
      Data[0].sigmaprof[i][j][2] = 0.0; // py*py
    }
  }
...
// initialize particle locations
  for (i = 0; i < Param[0].N; i++) {
      ...
      Particles[0][i].px = 0.0;
      Particles[0][i].py = 0.0;
      Particles[0][i].dx = 0.0;
      Particles[0][i].dy = 0.0;
      Particles[0][i].dpx = 0.0;
      Particles[0][i].dpy = 0.0;
...
void Update_Particles(param Param,particle* Particles, state State
  ...
  for (n=0 ; n<Param.N ; n++ ){
    ...
    sqrt2Ddt = pow(Particles[n].T, 0.5) * Param.sqrt2gammadt;
    Particles[n].dpx += sqrt2Ddt * gasdev();
    Particles[n].dpy += sqrt2Ddt * gasdev();

    Particles[n].dpx -= Param.gamma_dt * Particles[n].px;
    Particles[n].dpy -= Param.gamma_dt * Particles[n].py;

#ifndef NI
    // incorporate interactions
    Particles[n].dpx += Particles[n].ifx;
    Particles[n].dpy += Particles[n].ify;
#endif

    Particles[n].dx = Particles[n].px * Param.dt;
    Particles[n].dy = Particles[n].py * Param.dt;
  }
  ...
  for ( n=0 ; n<Param.N ; n++ ){
    ...
    Particles[n].px += Particles[n].dpx;
    Particles[n].py += Particles[n].dpy;
...
void Store_Parameters (int argc, char *argv[], param Param) {
  ...
  fprintf (Param.output_param, "gamma is %lg\n",           Param .gamma);
...
#endif
    Data[0].Jprof[j][k][2] += Particles[i].px*Param.dt;
    Data[0].Jprof[j][k][3] += Particles[i].py*Param.dt;
  }
}
...
void Update_Sigma(param Param,particle* Particles, data* Data) {
  long i; // particle index
  int j,k;// index to add to. From 0 to Param.Nbinx-1 and 0 to Param.Nbiny-1
  ...
  for (i=0; i<Param.N; i++){
    j = (int) (floor(Particles[i].x/Param.binwidthx)+EPS);
    k = (int) (floor(Particles[i].y/Param.binwidthy)+EPS);
    Data[0].sigmaprof[j][k][0] += Particles[i].px * Particles[i].px;
    Data[0].sigmaprof[j][k][1] += Particles[i].px * Particles[i].py;
    Data[0].sigmaprof[j][k][2] += Particles[i].py * Particles[i].py;
  }
}

void Store_Sigma(param Param,particle* Particles, state State, data* Data) {
  int n,j,k; // profile index
  
  for (n=0; n<3; n++){
    for (j=0; j<Param.Nbinx; j++){
      fprintf(Param.output_sigmaprof, "%.4f",State._time);
      for (k=0; k<Param.Nbiny; k++){
        fprintf(Param.output_sigmaprof, "\t%lg", Data[0].sigmaprof[j][k][n]);
        Data[0].sigmaprof[j][k][n]=0.0;
      }
      fprintf(Param.output_sigmaprof, "\n");
    }
  }
  fflush(Param.output_sigmaprof);
}
...
void Update_Data(param Param, particle* Particles, state* State, data* Data){
  if (State[0]._time > State[0].NextUpdateProf-EPS){
    ...
    Update_Sigma(Param, Particles, Data);
#ifndef NI 
...
void Store_Data(param Param, particle* Particles, state* State, data* Data){
  ...
  if (State[0]._time > State[0].NextStoreProf-EPS){
    ...
    Store_Sigma(Param, Particles, State[0], Data);
#ifndef NI 
