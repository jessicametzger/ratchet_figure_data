...
fig = plt.figure(figsize=(3.4, 3.4*.7))
gs = gridspec.GridSpec(5, 2, height_ratios=[.52,.1, .8, .15, .7], width_ratios = [1,0.04], hspace=0, wspace=0.1)
...
cbar.set_label(r'$\hat{\sigma}(\mathbf{r})$',rotation='horizontal')
...
axs[2].plot(np.linspace(0,p.tf,NstoreProf+1), [0]+list(epr_series), color='black')

fig.add_subplot(gs[0, 1]).axis('off')

# T(x) plot
axs[0].plot(xs_prof+dx_prof/2.0, Ts, color='green', lw=2)

# axis limits
axs[2].set_xlim(xmin=0,xmax=p.tf)
axs[2].set_ylim(ymin=0, ymax=np.max(epr_series)*1.1)
axs[0].set_ylim(ymax=19,ymin=0)

# axis tick params
axs[2].set_xticks([0,5e4,1e5],labels=['0',r'$5 \times 10^4$', r'$10^5$'])
...
axs[2].set_ylabel(r'$\Sigma(t)$',rotation='horizontal')
...
axs[2].yaxis.set_label_coords(-0.05,0.73)
axs[0].xaxis.set_label_coords(0.95,0.22)
...
plt.subplots_adjust(left=0.091,bottom=0.08,right=0.9,top=0.98)

plt.savefig('companion_EPR.pdf',dpi=1000)
plt.savefig('companion_EPR.png',dpi=1000)
