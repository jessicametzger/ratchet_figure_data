...
void Initialize_parameters (int argc, char *argv[], param* Param, state* State, data* Data, particle** Particles
  ...
  strcat(command_base, "Nstep_U ");                  argctarget ++;
  strcat(command_base, "U_xs ");                     argctarget ++;
  strcat(command_base, "Us ");                       argctarget ++;
#ifndef NI
  ...
  Param[0].Ts                 = argv[i]                            ; i++ ;
  Param[0].UParam.Nstep       = (int) strtod(argv[i], NULL)        ; i++ ;
  Param[0].Uxs                = argv[i]                            ; i++ ;
  Param[0].Us                 = argv[i]                            ; i++ ;
#ifndef NI
  ...
  // cubic potential landscape
  Param[0].UParam.L             = Param[0].Lx;
  Param[0].UParam.xs            = (double*) calloc(Param[0].UParam.Nstep, sizeof(double));
  Param[0].UParam.fs            = (double*) calloc(Param[0].UParam.Nstep, sizeof(double));
  split_vec(Param[0].UParam.Nstep, &Param[0].UParam.xs, Param[0].Uxs, ',');
  split_vec(Param[0].UParam.Nstep, &Param[0].UParam.fs, Param[0].Us, ',');
  for (i=0; i<Param[0].UParam.Nstep; i++){ // scale by dt
    Param[0].UParam.fs[i] = Param[0].UParam.fs[i] * Param[0].dt;
  }
  compute_coefs_cub(&Param[0].UParam);
...
// initialize particle locations & data
  for (i = 0; i < Param[0].N; i++) {
...
// force landscape (to model discretization)
double Fofx_dt(fparam_cub UParam, double x, double y){
  return -dfdx_cub(UParam, x);
}
...
void Update_Particles(param Param,particle* Particles, state State
  ...
  for (n=0 ; n<Param.N ; n++ ){
    ...
    Particles[n].Fx = Fofx_dt(Param.UParam, Particles[n].x, Particles[n].y);
...
void Store_Parameters (int argc, char *argv[], param Param) {
  ...
  fprintf( Param .output_param, "Nstep_U is %d\n",         Param .UParam .Nstep);
  fprintf( Param .output_param, "Uxs is %s\n",             Param .Uxs);
  fprintf( Param .output_param, "Us is %s\n",              Param .Us);
#ifndef NI
