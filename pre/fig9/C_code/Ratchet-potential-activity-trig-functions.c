...
void split_mode_vec(int Nexpect, double** output, const char* input){
    char delimiter=',';
    int count = 1;
    for (const char *c = input; *c != '\0'; c++) if (*c == delimiter) count ++;
    if (count!=Nexpect) printf("input list has length %d, not equal to Nexpect=%d\n", count, Nexpect); // exit(1);
...
void Initialize_parameters(int argc, char* argv[],
  ...
  */
  ...
  strcat(command_base, "Nmode "); argctarget ++;
  strcat(command_base, "v0 "); argctarget ++;
  strcat(command_base, "As "); argctarget ++;
  strcat(command_base, "dAs "); argctarget ++;
  strcat(command_base, "Bs "); argctarget ++;
  strcat(command_base, "dBs "); argctarget ++;
  ...
  Param[0].Nmode              = (int) strtod(argv[i], NULL); i++;
  ...
  // parameters of activity and potential landscapes
  Param[0].As                 = (double*) calloc(Param[0].Nmode,sizeof(double));
  Param[0].dAs                = (double*) calloc(Param[0].Nmode,sizeof(double));
  Param[0].Bs                 = (double*) calloc(Param[0].Nmode,sizeof(double));
  Param[0].dBs                = (double*) calloc(Param[0].Nmode,sizeof(double));
  Param[0].cs                 = (double*) calloc(Param[0].Nmode,sizeof(double));
  Param[0].v0_As              = (double*) calloc(Param[0].Nmode,sizeof(double));
  Param[0].F_Bs               = (double*) calloc(Param[0].Nmode,sizeof(double));
  Param[0].dAs_2pi            = (double*) calloc(Param[0].Nmode,sizeof(double));
  Param[0].dBs_2pi            = (double*) calloc(Param[0].Nmode,sizeof(double));
  split_mode_vec(Param[0].Nmode, &Param[0].As, As);
  split_mode_vec(Param[0].Nmode, &Param[0].dAs, dAs);
  split_mode_vec(Param[0].Nmode, &Param[0].Bs, Bs);
  split_mode_vec(Param[0].Nmode, &Param[0].dBs, dBs);
  for (i=0; i<Param[0].Nmode; i++){
    Param[0].v0_As[i]         = Param[0].v0*Param[0].As[i];
    Param[0].cs[i]            = 2*M_PI*(i+1)*Param[0].P/Param[0].L;
    Param[0].dAs_2pi[i]       = Param[0].dAs[i]*2*M_PI*Param[0].P;
    Param[0].dBs_2pi[i]       = Param[0].dBs[i]*2*M_PI*Param[0].P;
    Param[0].F_Bs[i]          = 2*M_PI*(i+1)*Param[0].Bs[i]*Param[0].P/Param[0].L;
  }
...
// Make a periodic velocity field made of Nmode Fourier modes.
double vofx_function(double x, param Param){
  double v=Param.v0;
  for (int i=0; i<Param.Nmode; i++){
    v += Param.v0_As[i] * sin(Param.cs[i]*x + Param.dAs_2pi[i]);
  }
  return v;
}

// Force_potential computes the force exerted on a particle by the potential
double Force_potential(double x, param Param){
  double F=0.0;
  for (int i=0; i<Param.Nmode; i++){
    F -= Param.F_Bs[i] * cos(Param.cs[i]*x + Param.dBs_2pi[i]);
  }
  return F;
}
...
void Store_Parameters(int argc, char* argv[], FILE* output_param, param Param,
  ...
  fprintf(output_param,"Nmode is %d\n", Param.Nmode);
  fprintf(output_param,"v0 is %lg\n", Param.v0);
  fprintf(output_param,"As is ");
  for (i=0; i<Param.Nmode; i++) {
    if (i>0) fprintf(output_param,",");
    fprintf(output_param,"%lg",Param.As[i]);
  }
  fprintf(output_param,"\ndAs is ");
  for (i=0; i<Param.Nmode; i++) {
    if (i>0) fprintf(output_param,",");
    fprintf(output_param,"%lg",Param.dAs[i]);
  }
  fprintf(output_param,"\nBs is ");
  for (i=0; i<Param.Nmode; i++) {
    if (i>0) fprintf(output_param,",");
    fprintf(output_param,"%lg",Param.Bs[i]);
  }
  fprintf(output_param,"\ndBs is ");
  for (i=0; i<Param.Nmode; i++) {
    if (i>0) fprintf(output_param,",");
    fprintf(output_param,"%lg",Param.dBs[i]);
  }
  fprintf(output_param,"\nP is %d\n", Param.P);
