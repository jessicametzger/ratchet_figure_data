...
typedef struct param{
  ...
  int Nmode;                       // number of trigonometric modes for activity landscape
  double v0;                       // set the scale of the particle speeds
  double* As;                      // coefficient of the fourier modes for activity landscape
  double* dAs;                     // phase shifts of v(x) fourier modes
  double* Bs;                      // coefficient of the fourier modes for potential landscape
  double* dBs;                     // phase shifts of U(x) fourier modes
  double* cs;                      // x coefficient to go inside fourier modes
  double* v0_As;                   // v0*as/2
  double* F_Bs;                    // coefficient of the force = bs*2*n*pi*P/L
  double* dAs_2pi;                 // 2*pi*das*P
  double* dBs_2pi;                 // 2*pi*dbs*P
...
#include "Ratchet-potential-activity-trig-functions.c" // our methods
