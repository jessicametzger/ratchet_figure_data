...
typedef struct param{
  ...
  fparam_cub vParam;         // parameters of activity landscape (included in inhom_fluct.c)
  char* vs;
  char* vxs;
  ...
  FILE* output_FAprof;       // File where active force profile is stored
  FILE* output_Fintprof;       // File where active force profile is stored
  FILE* output_sigmaAprof;   // File where active stress profile is stored
  FILE* output_sigmaIKprof;  // File where Irving-Kirkwood stress profile is stored
...
#include "PFs.c"
#include "./ABP-v_step-2D-functions.c" // our methods
